...
def track_edge(im,start=None):
    ...
    #loop until break
    while True:
        #set next direction to look, counterclockwise from where we came
        checkdir = (dir+3) % 4
...
#reduce edge to be 8-connected where possible
def reduce_edge(chain):
    #keep starting pixel info - it can't be redundant
    chain8 = [chain[i] for i in range(2)]
    i,j = 2,3
    
    while j<len(chain):
        #the edge is redundant if the difference of the directions gone is a negative odd number
        #because of the direction we're traversing, 0->3 and 3->0 combos are a bit tricky
...
def label_components(im):
    ...
    return l_im[1:-1,1:-1],equiv    #get rid of padding